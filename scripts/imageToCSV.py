...
def crop_tomato(image, bbox):
    """
    Crop tomato from image using bounding box coordinates.
    """
...
def resize_image(image, target_size=(64, 64)):
    """
    Resize image to fixed size for consistent feature vector length.
    """
...
def create_tomato_csv(images_dir, labels_dir, output_csv, target_size=(64, 64), batch_size=50):
    """
    Extract tomatoes from images and create CSV file with memory-efficient batch processing.
    
    Parameters:
    - images_dir: Directory containing images
    - labels_dir: Directory containing YOLO label files
    - output_csv: Output CSV file path
    - target_size: Size to resize cropped tomatoes (width, height)
    - batch_size: Number of tomatoes to process before writing to disk
    """
...
if __name__ == "__main__":
    # Configure these paths according to your dataset structure
    IMAGES_DIR = "../assets/image_dataset/train/images"  # Directory with .jpg/.png files
    LABELS_DIR = "../assets/image_dataset/train/labels"  # Directory with .txt label files
    OUTPUT_CSV = "../assets/image_dataset/train/tomatoes_dataset.csv"
    
    # Target size for resized tomatoes (smaller = fewer features, faster processing)
    # 32x32 = 3,072 features - very fast, low memory
    # 64x64 = 12,288 features - good balance
    # 128x128 = 49,152 features - high detail but slow and memory intensive
    TARGET_SIZE = (64, 64)  # Reduced default size for better performance
    
    # Batch size: number of tomatoes to accumulate before writing to disk
    BATCH_SIZE = 30  # Adjust based on your RAM (lower = less memory usage)
    
    # Create the CSV file
    create_tomato_csv(IMAGES_DIR, LABELS_DIR, OUTPUT_CSV, TARGET_SIZE, BATCH_SIZE)